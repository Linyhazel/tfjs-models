...
var stopBtn = document.getElementById("stop-recording");//newly added
...
function stopRecord(callback) {
	//stop stacking coordinates, pass list of coordinates to rnn model
	start_recording = 0;
	saved_coordinates = saved_buffer;
	var width = rightmost_x - leftmost_x;
	var height = downmost_y - upmost_y;
	//saved_buffer = []
    //leftmost_x = 960
    //rightmost_x = 0
    //upmost_y = 540
    ...
    var i = 0;
    var j = 0;
    var len = saved_coordinates.length;
    if(len<40){
    	alert("Video being evaluated is too short!");
    	return;
    }
    //normalize data
    for(i=0; i<len; i++){
    ...
    for(i=0; i<len-40+1; i++){
    	coordinatesArray.push(saved_coordinates.slice(i, i+40));
    }

    console.log(coordinatesArray.length);
    var predict_y = model.predict(tf.tensor3d(coordinatesArray).reshape([coordinatesArray.length, 40, 34]));
    //var py = tf.tensor1d(predict_y.dataSync()).reshape([coordinatesArray.length, 1, 6]);
    console.log(predict_y.dataSync().max());
}
