...
async function initModel(){
  ...
  model = await tf.loadLayersModel('http://localhost:8080/model_regression/model.json');
...
function translate(array32){
  var activity_name = ""
  var pactivity = Math.round(array32[0]), pscore = Math.round(array32[1]); //activity: 0=underhand-throw    1=run    2=jump    3=catch
  if(pscore < 0){
    pscore = 0;
  }
  else if(pscore > 5){
    ...
    poses.forEach(({score, keypoints}) => {
      if (score >= minPoseConfidence) {
        if (1) {
          ...
          if(saved_coordinates.length == 40){
            //call model
            if(count == 0){
              ...
              for(j=0; j<34; j++){
                  if(j<14){
                    saved_coordinates[39][j] = (saved_coordinates[39][j]-leftmost_x)/width;
                  }else{
                    saved_coordinates[39][j] = (saved_coordinates[39][j]-upmost_y)/height;          
                  }
                }
            }
            
            var predict_y = model.predict(tf.tensor2d(saved_coordinates).reshape([1, 40, 34]));
            var predict_values = translate(predict_y.dataSync());
            console.log(predict_y.dataSync());
            document.getElementById('genreLbl').innerHTML = predict_values[0].toString();
            document.getElementById('scoreLbl').innerHTML = predict_values[1].toString();

            //slice array
            count++;
            saved_coordinates = saved_coordinates.slice(1, 40);
          }
        }
