...
function translate(array32){
  var activity_name = ""
  var pactivity = Math.round(array32[0]), pscore = Math.round(array32[1]); //activity: 0=catch    1=jump    2=run    3=underhand-throw
  if(pscore < 0){
    pscore = 0;
  }
  else if(pscore > 5){
...
videoInput.addEventListener('ended', (event) => {
  ...
  var pad_no = 90-saved_coordinates.length;
  if(pad_no>0){
    var padding = tf.zeros([pad_no, 34]);
    input = tf.concat([tf.tensor2d(saved_coordinates), padding], 0);
  }
  else{
    input = tf.tensor2d(saved_coordinates.slice(-91, -1));
  }

  var predict_y = model.predict(input.reshape([1, 90, 34]));
